...
public final class PermissionDatabase implements Serializable {
    ...
    public PermissionDatabase() {
        if (!playerDir.exists() || !Files
            .exists(Paths.get("reformcloud/addons/permissions/config.json"))) {
            playerDir.mkdirs();
        ...
        this.permissionCache = Configuration
            .parse(Paths.get("reformcloud/addons/permissions/config.json"))
        ...
        ReformCloudController.getInstance().getNettyHandler()
            ...
            .registerQueryHandler("QueryGetPermissionHolder",
                new PacketInQueryGetPermissionHolder())
            ...
            for (File file : playerDir.listFiles()) {
                if (file.getName().endsWith(".json") && file.getName().replace(".json", "")
                    ...
                    try {
                        ...
                        PermissionHolder permissionHolder1 = configuration
                            .getValue("holder", TypeTokenAdaptor.getPERMISSION_HOLDER_TYPE());
                        List<String> copy = new ArrayList<>(
                            permissionHolder1.getPermissionGroups().keySet());
                        ...
                        if (permissionHolder1.getPermissionGroups().size() == 0) {
                            permissionHolder1.getPermissionGroups()
                                .put(this.permissionCache.getDefaultGroup().getName(), -1L);
                            this.updatePermissionHolder(permissionHolder1);
                        }

                        this.cachedPermissionHolders
                            .put(permissionHolder.getUniqueID(), permissionHolder1);
    ...
    public void updatePermissionGroup(PermissionGroup permissionGroup) {
        if (permissionGroup.getName()
            .equalsIgnoreCase(this.permissionCache.getDefaultGroup().getName())) {
            this.permissionCache.setDefaultGroup(permissionGroup);
            return;
        }

        List<PermissionGroup> copyOf = new ArrayList<>(
            this.permissionCache.getAllRegisteredGroups());
    ...
    public void update() {
        new Configuration().addValue("permissionConfig", this.permissionCache)
            .write(Paths.get("reformcloud/addons/permissions/config.json"));
        ReformCloudController.getInstance().getChannelHandler().sendToAllSynchronized(
    ...
    private PermissionGroup getPermissionGroup(String name) {
        return this.getAllGroups().stream().filter(e -> e.getName().startsWith(name)).findFirst()
            .orElse(null);
    }
