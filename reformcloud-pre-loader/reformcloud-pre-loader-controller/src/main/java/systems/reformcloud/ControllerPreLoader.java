...
public final class ControllerPreLoader implements Serializable {
    ...
    private static void deleteExistingVersion() {
        ...
        for (File file : files) {
            if (file.getName().contains("ReformCloudController") && file.getName().endsWith(".jar") && !file.getName().contains(CommonLoader.getCurrentFallbackVersion()))
                if (file.delete())
                    break;
        }
    }
