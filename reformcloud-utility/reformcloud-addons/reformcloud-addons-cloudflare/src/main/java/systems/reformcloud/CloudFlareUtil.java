...
public final class CloudFlareUtil implements Serializable {
    ...
    private synchronized void deleteRecord(Result result) {
        try {
            HttpURLConnection httpURLConnection = (HttpURLConnection) new URL(
                    "https://api.cloudflare.com/client/v4/zones/" + zoneID + "/dns_records/" + result.getId()
            ).openConnection();
    ...
    public void createProxyEntry(ProxyInfo proxyInfo) {
        ...
        CloudFlareConfig.CloudFlareGroup cloudFlareGroup = this.findGroup(proxyInfo.getProxyGroup().getName());
        ...
        ADNSDefaultRecord.SRVRecord srvRecord = new ADNSDefaultRecord.SRVRecord(
                ...
                "SRV 1 1 " + proxyInfo.getPort() + " " + proxyInfo.getCloudProcess().getClient() + "." + this.cloudFlareConfig.getDomain(),
                "_minecraft",
                "_tcp",
                cloudFlareGroup.getSubDomain().startsWith("@") ? this.cloudFlareConfig.getDomain() : cloudFlareGroup.getSubDomain(),
                1,
                1,
                proxyInfo.getPort(),
