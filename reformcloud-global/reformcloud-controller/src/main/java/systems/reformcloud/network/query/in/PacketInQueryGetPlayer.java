...
public final class PacketInQueryGetPlayer implements Serializable, NetworkQueryInboundHandler {
    ...
    public void handle(Configuration configuration, UUID resultID) {
        if (configuration.contains("uuid")) {
            OfflinePlayer offlinePlayer = ReformCloudController.getInstance().getPlayerDatabase().getOfflinePlayer(
            ...
            DefaultPlayer defaultPlayer = configuration.getValue("player", TypeTokenAdaptor.getDEFAULT_PLAYER_TYPE());
            OfflinePlayer offlinePlayer = ReformCloudController.getInstance().getPlayerDatabase().getOfflinePlayer(defaultPlayer);
