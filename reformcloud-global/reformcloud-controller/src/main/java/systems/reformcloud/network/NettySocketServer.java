...
public final class NettySocketServer extends ChannelInitializer<Channel> implements AutoCloseable,
    Serializable {

    private SslContext sslContext;

    private EventLoopGroup workerGroup = ReformCloudLibraryService
        .eventLoopGroup();

    private EventLoopGroup bossGroup =
        ReformCloudLibraryService.eventLoopGroup();

    /**
     * Prepares a socket server by using io.network and binds him using {@link ServerBootstrap}
     * synchronized on the main thread. This is blocking and will await the bind. If ssl is enabled
     * the {@link SslContext} will be prepared first by using an self- signed certificate using an
     * SslContextBuilder {@link SslContextBuilder#forServer(PrivateKey, X509Certificate...)}. Please
     * note all ChannelOptions's below in the main source. If an handler connects the cloud will
     * prepare the channel by using the ReformCloudLibraryService#prepareChannel method. If an
     * exceptions occurs it will be catch by {@link Throwable} below and printed in the console. If an
     * error occurs, you can contact the support.
     *
     * @param ssl If this is {@code true} the ssl context will be enabled and a self-signed
     * certificate will be added to every channel, which tries to connect to the
     * ReformCloudController
     * @param ethernetAddress Main address where the cloud tries to bind the socket server to. Please
     * make sure that the port is not in use, yet
     */
    ...
    private boolean isIpAllowed(String ip) {
        List<String> ips = new ArrayList<>(
            ReformCloudController.getInstance().getCloudConfiguration().getIpWhitelist());
        ReformCloudController.getInstance().getInternalCloudNetwork().getClients().values()
            .forEach(client -> ips.add(client.getIp()));
