...
public final class CommandHelp extends Command implements Serializable {
    ...
    public void executeCommand(CommandSender commandSender, String[] args) {
        ...
        ReformCloudClient.getInstance().getCommandManager().getCommands().forEach(command -> commandSender.sendMessage("   - " + command.getName() + " | Aliases: §e" + Arrays.asList(command.getAliases()) + "§r | Description: §3" + command.getDescription() + "§r | Permission: " + (command.getPermission() == null ? "none" : command.getPermission())));
