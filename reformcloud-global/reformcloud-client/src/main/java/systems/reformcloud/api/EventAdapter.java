...
public final class EventAdapter implements Serializable, EventHandler {
    ...
    public void channelDisconnected(ChannelHandlerContext channelHandlerContext) {
        final String serviceName = ReformCloudClient.getInstance().getChannelHandler()
        ...
        if (serviceName != null) {
            ...
            if (serviceName.equals("ReformCloudController")) {
                ...
                ReformCloudClient.getInstance().getCloudProcessScreenService()
                    .getRegisteredServerProcesses()
                    .forEach(
                        cloudServerStartupHandler -> cloudServerStartupHandler.shutdown(false));
