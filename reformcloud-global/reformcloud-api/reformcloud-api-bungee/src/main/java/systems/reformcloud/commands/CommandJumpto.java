...
public final class CommandJumpto extends Command implements TabExecutor {
    ...
    public void execute(CommandSender commandSender, String[] strings) {
        ...
        if (strings.length == 1) {
            ...
            if (BungeecordBootstrap.getInstance().getProxy().getPlayer(strings[0]) != null) {
                proxiedPlayer.connect(BungeecordBootstrap.getInstance().getProxy().getServerInfo(
                    BungeecordBootstrap.getInstance().getProxy().getPlayer(strings[0]).getServer()
                        .getInfo().getName()));
            ...
            if (BungeecordBootstrap.getInstance().getProxy().getServers().get(strings[0]) != null) {
                proxiedPlayer.connect(
                    BungeecordBootstrap.getInstance().getProxy().getServers().get(strings[0]));
                proxiedPlayer.sendMessage(ChatMessageType.CHAT, TextComponent.fromLegacyText(
                    ReformCloudAPIBungee.getInstance().getInternalCloudNetwork()
                        .getMessage("internal-api-bungee-command-jumpto-success")));
            }
    ...
    public Iterable<String> onTabComplete(CommandSender commandSender, String[] strings) {
        ...
        BungeecordBootstrap.getInstance().getProxy().getPlayers().stream()
            .filter(e -> e.getName().startsWith(stringBuilder.substring(0)))
            .forEach(e -> iterable.add(e.getName()));
