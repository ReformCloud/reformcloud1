...
public final class CommandHub extends Command {
    ...
    public void execute(CommandSender commandSender, String[] strings) {
        ...
        if (ReformCloudAPIBungee.getInstance().getInternalCloudNetwork().getServerProcessManager().getRegisteredServerByName(proxiedPlayer.getServer().getInfo().getName()) != null
                && ReformCloudAPIBungee.getInstance().getInternalCloudNetwork().getServerProcessManager().getRegisteredServerByName(proxiedPlayer.getServer().getInfo().getName()).getServerGroup().getServerModeType().equals(ServerModeType.LOBBY)) {
        ...
        if (fallback == null)
            proxiedPlayer.sendMessage(ChatMessageType.CHAT, TextComponent.fromLegacyText(ReformCloudAPIBungee.getInstance().getInternalCloudNetwork().getMessage("internal-api-bungee-command-hub-not-available")));
        else
