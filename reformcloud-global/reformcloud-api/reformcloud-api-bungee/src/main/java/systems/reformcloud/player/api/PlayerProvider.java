...
public final class PlayerProvider implements Serializable, IDefaultPlayerProvider {
    ...
    public void sendPlayer(UUID uniqueID, String name) {
        if (BungeecordBootstrap.getInstance().getProxy().getPlayer(uniqueID) != null) {
            ProxiedPlayer proxiedPlayer = BungeecordBootstrap.getInstance().getProxy().getPlayer(uniqueID);
    ...
    public void kickPlayer(UUID uniqueID, String reason) {
        if (BungeecordBootstrap.getInstance().getProxy().getPlayer(uniqueID) != null) {
            ProxiedPlayer proxiedPlayer = BungeecordBootstrap.getInstance().getProxy().getPlayer(uniqueID);
            proxiedPlayer.disconnect(reason == null ? TextComponent.fromLegacyText("§cNo reason defined")
