    ...
    public void handle(final CloudServerAddEvent event) {
        VelocityBootstrap.getInstance().getProxyServer().registerServer(
            new ServerInfo(
                ...
                new InetSocketAddress(event.getServerInfo().getHost(),
                    event.getServerInfo().getPort())
            ));

        if (event.getServerInfo().getServerGroup().getServerModeType()
            ...
            if (proxiedPlayer.hasPermission("reformcloud.notify.server")) {
                proxiedPlayer.sendMessage(TextComponent
                    .of(ReformCloudAPIVelocity.getInstance().getInternalCloudNetwork()
                        .getMessage("internal-api-bungee-startup-server")
    ...
    public void handle(final CloudProxyAddEvent event) {
        VelocityBootstrap.getInstance().getProxyServer().getAllPlayers().forEach(proxiedPlayer -> {
            if (proxiedPlayer.hasPermission("reformcloud.notify.proxy")) {
                proxiedPlayer.sendMessage(TextComponent
                    .of(ReformCloudAPIVelocity.getInstance().getInternalCloudNetwork()
                        .getMessage("internal-api-bungee-startup-proxy").replaceAll("%proxy-name%",
                            event.getProxyInfo().getCloudProcess().getName())));
            }
        });
    }
    ...
    public void handle(final CloudServerRemoveEvent event) {
        VelocityBootstrap.getInstance().getProxyServer().unregisterServer(
            VelocityBootstrap.getInstance().getProxyServer()
                .getServer(event.getServerInfo().getCloudProcess().getName()).get().getServerInfo()
        );

        if (event.getServerInfo().getServerGroup().getServerModeType()
            .equals(ServerModeType.LOBBY)) {
            VelocityBootstrap.getInstance().getProxyServer().getConfiguration()
                .getAttemptConnectionOrder().remove(
                event.getServerInfo().getCloudProcess().getName()
            );
        }
        ...
        VelocityBootstrap.getInstance().getProxyServer().getAllPlayers().forEach(proxiedPlayer -> {
            if (proxiedPlayer.hasPermission("reformcloud.notify.server")) {
                proxiedPlayer.sendMessage(TextComponent
                    .of(ReformCloudAPIVelocity.getInstance().getInternalCloudNetwork()
                        .getMessage("internal-api-bungee-remove-server").replaceAll("%server-name%",
                            event.getServerInfo().getCloudProcess().getName())));
            }
        });
    }
    ...
    public void handle(final CloudProxyRemoveEvent event) {
        VelocityBootstrap.getInstance().getProxyServer().getAllPlayers().forEach(proxiedPlayer -> {
            if (proxiedPlayer.hasPermission("reformcloud.notify.proxy")) {
                proxiedPlayer.sendMessage(TextComponent
                    .of(ReformCloudAPIVelocity.getInstance().getInternalCloudNetwork()
                        .getMessage("internal-api-bungee-remove-proxy").replaceAll("%proxy-name%",
                            event.getProxyInfo().getCloudProcess().getName())));
            }
        });
    }
